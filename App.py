...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills,
    ...
    connection.commit()
st.set_page_config(
    page_title="Smart Vision Resume Analyzer",
    page_icon='Logo\SV.ico',
)
def run():
    st.title("Smart Vision Resume Analyser")
    st.sidebar.markdown("# Choose User")
    activities = ["Normal User", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    img = Image.open('Logo\SV.jpg')
    img = img.resize((250, 250))
    st.image(img)
    db_sql = """CREATE DATABASE IF NOT EXISTS SRA;"""
    cursor.execute(db_sql)
    connection.select_db("smartvision")
    DB_table_name = 'user_data'
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'smart_vision' and ad_password == '5259':
                st.success("Welcome Admin SV")
                ...
                st.subheader("📊 **Bar-Chart for Predicted Field Recommendations**")
                fig = px.bar(plot_data, x=labels, y=values, title='Predicted Field according to the Skills')
                st.plotly_chart(fig)

                labels = plot_data['User_level'].unique()
                values = plot_data['User_level'].value_counts().values
                st.subheader("📊 **Bar-Chart for User's Experienced Level**")
                fig = px.bar(plot_data, x=labels, y=values, title="Bar-Chart for User's Experienced Level")
                st.plotly_chart(fig)
